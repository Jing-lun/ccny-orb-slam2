...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, string tat)
{
    if(mSensor!=RGBD)
    {
    ...
    return mpTracker->GrabImageRGBD(im,depthmap,timestamp,tat);
}
...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const Eigen::Quaterniond &q, const Eigen::Matrix<double,3,1> &imu_a, const Eigen::Matrix<double,3,1> &imu_w, const Eigen::Matrix<double,3,1> &odom)
{
    if(mSensor!=RGBD)
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
    ...
    return mpTracker->GrabImageRGBD(im,depthmap,timestamp,q,imu_a,imu_w,odom);
}
