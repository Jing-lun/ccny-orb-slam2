...
public:
    ...
    cv::Mat TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, string tat);
    // 把imu直接输出的四元数、加计、陀螺数据当做参数传递
    cv::Mat TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const Eigen::Quaterniond &q, const Eigen::Matrix<double,3,1> &imu_a, const Eigen::Matrix<double,3,1> &imu_w, const Eigen::Matrix<double,3,1> &odom);
